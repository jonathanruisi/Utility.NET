...
namespace JLR.Utility.NET
{
	...
	/// </typeparam>
	public struct DiscreteRange<T> : IEquatable<DiscreteRange<T>>, IEnumerable<T> where T : struct
	{
		#region Properties
		...
		#region Constructors
		/// <summary>
		/// Initializes a new instance of the <see cref="DiscreteRange{T}"/> structure.
		/// </summary>
		...
		/// </param>
		public DiscreteRange(T minimum, T maximum, T increment = default(T))
		{
			Minimum   = minimum;
			Maximum   = maximum;
			Increment = increment;
		...
		{
			if (obj is DiscreteRange<T>)
				return Equals((DiscreteRange<T>)obj);
			return false;
		}
		...
		public static bool operator ==(DiscreteRange<T> lhs, DiscreteRange<T> rhs)
		{
